...
import static com.mert.secretofcolors.SplashActivity.renkdiziint;

public class CameraActivity4 extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if (durum) {
            ...
            rgbint = 65536 * ortalama[0] + 256 * ortalama[1] + ortalama[2];
    ...
    public void FrameCounter(int colorListIndex, int[] ortalama){
        ...
        count++;
        int globaldeger = colorListIndex;
        if (count==17 && ilk==0){
            ilk++;
            count++;
            playProcess(colorListIndex);
        }
        else if(count%245==0 ){ //90 da iken 3,8 snde bir dönüyor 120 iken 4,5 sn bir dönüyor 250 10,6sn
            playProcess(colorListIndex);
    ...
    public void playProcess(int a){
        ...
        try {
            mediaPlayer = MediaPlayer.create(this, Uri.parse("android.resource://"+getPackageName()+"/raw/oyun"+a)); //sarki 1 kirmizi
            mediaPlayer.start();
        }
